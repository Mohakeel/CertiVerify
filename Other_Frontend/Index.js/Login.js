...
function animateNetwork() {
  updateNodes();
  drawNetwork();
  animId = requestAnimationFrame(animateNetwork);
}
...
function validateSignIn() {
  ...
  if (!email) {
    showError(emailInput, emailError, 'Institutional email is required.');
    valid = false;
...
signinBtn.addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    showToast('✓  Signed in successfully! Redirecting to vault…');
  }, 1800);
});
